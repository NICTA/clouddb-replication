...
install_ssh_policy()
{
  ...
  # Deploy ssh policy
  scp -r config.ssh.template root@$1:~/.ssh/config
  scp -r dbadmin.pem.template root@$1:~/.ssh/dbadmin.pem && \
...
install_olio_sys()
{
  ...
  && gem install rake thin RubyInline image_science rcov will_paginate -V --no-ri --no-rdoc"
